...
def most_sim_word(word, choices, semantic_descriptors, similarity_fn):
    ...
    for c in choices:
        #if the similarity cannot be computed (KeyError)
        #set the similarity to -inf
        try:
            ...
            if updated_similarity > similarity:
                ...
                choice = c
    #if every choice in choices yield to -inf meaning that none is computable
    #return ''
...
def run_sim_test(filename, semantic_descriptors, similarity_fn):
    ...
    65.0
    '''
    #retrieve the file in lines
    fobj = open(filename, 'r')
    content = fobj.readlines()
    fobj.close()
    num_correct = 0
    total_num = 0
    #check the correctness of each question
    for line in content:
        words = line.split()
        if most_sim_word(words[0], words[2:len(words)], semantic_descriptors, similarity_fn) == words[1]:
            ...
            total_num += 1
    #divide the number of correct ones by the total number and times 100 to get the percentage
...
def generate_bar_graph(similarity_fns, filename):
    ...
    scores = []
    fns = []
    #append the performance of each function into the scores list
    #append each function name in string into the fns list
